...
def transform_c(df, cols, logger):
    
    for c in cols:
        logger.info(c)
        if df[c].isna().sum() > 0:
            stats = print_descriptive_metrics(df,c,logger)
            logger.info(stats[0])
...
'''
def print_descriptive_metrics(df,col,logger):
    import numpy as np
...
'''
def plot_clustering_silhouette(X_std,labels,k,xlab,ylab):
    import matplotlib.pyplot as plt
    import numpy as np
    import itertools
...
'''
def plot_clusters(X,y_kmeans,kmeans,n_clusters,xlab,ylab):
    import numpy as np
    import matplotlib.pyplot as plt
    
    colors = ['red','blue','green','cyan','magenta','yellow','black','violet','olive','lime']
    for n in range(0,n_clusters):
        plt.scatter(X[y_kmeans == n, 0], X[y_kmeans == n, 1], s = 100, c = colors[n], label = f'Cluster {str(n+1)}')
        
    plt.scatter(kmeans.cluster_centers_[:, 0], kmeans.cluster_centers_[:, 1], s = 300, c = colors[n_clusters], label = 'Centroids')
...
'''
def save_model(model, filename, logger):
    import pickle
    import os
